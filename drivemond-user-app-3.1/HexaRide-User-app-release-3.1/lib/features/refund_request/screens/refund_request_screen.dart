...
class _RefundRequestScreenState extends State<RefundRequestScreen> {
            ...
            child: GetBuilder<RefundRequestController>(builder: (refundRequestController){
              return SingleChildScrollView(
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  if(refundRequestController.isReasonLoading)
                    Padding(
                      padding: const EdgeInsets.all(Dimensions.paddingSizeDefault),
                      child: Center(child: CircularProgressIndicator(color: Theme.of(context).primaryColor)),
                    )
                  else if((refundRequestController.refundReasonModel?.data?.isEmpty ?? true))
                    InkWell(
                      onTap: () => refundRequestController.getParcelRefundReasonList(),
                      child: Padding(
                        padding: const EdgeInsets.all(Dimensions.paddingSizeDefault),
                        child: Center(child: Text('${'no_data_found'.tr} - ${'tap_to_retry'.tr}',
                  ...
                  ListView.builder(
                    ...
                    shrinkWrap: true,
                    itemCount: refundRequestController.refundReasonModel?.data?.length ?? 0,
  ...
  void _onSubmit() {
    ...
    final reasons = refundRequestController.refundReasonModel?.data;
    final selectedIndex = refundRequestController.parcelRefundReasonCauseCurrentIndex;
    final bool hasValidReason = reasons != null && selectedIndex >= 0 && selectedIndex < reasons.length;
    String refundReason = hasValidReason ? reasons[selectedIndex] : '';
    String refundNote = refundNoteTextController.text;
    double approximatePrice = double.tryParse(productApproximatePriceController.text) ?? 0;

    if(!hasValidReason){
      showCustomSnackBar('select_refund_reason'.tr);

    }else if(productApproximatePriceController.text.trim().isEmpty || approximatePrice <= 0){
      ...
      refundRequestController.sendRefundRequest(
        ...
        productApproximatePrice: approximatePrice,
      );
    }
  }
}
