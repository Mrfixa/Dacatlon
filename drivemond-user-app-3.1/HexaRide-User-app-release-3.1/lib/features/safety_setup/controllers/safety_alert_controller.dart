...
class SafetyAlertController extends GetxController implements GetxService {
  ...
  // True until the first precaution-list fetch completes, so the screen shows a
  // shimmer while loading and a retry state on failure (not an endless shimmer).
  bool isPrecautionLoading = true;
  ...
  void checkDriverNeedSafety() async{
    ...
    // Gate on location permission before reading position, matching the rest of
    // the app; bail out quietly if the user hasn't granted it.
