...
class ProfileController extends GetxController implements GetxService{
  ...
  Future<Response> profileOnlineOffline( bool value) async {
    ...
    // showing Online while sending no location and spawning a permission dialog
    // every 10s. So require background access up front, not just foreground.
    if (isOnline == "0") {
      ...
      if (!alwaysPermission.isGranted) {
        ...
        if (!alwaysPermission.isGranted) {
          showCustomSnackBar('location_always_permission_required'.tr);
          return Response(statusCode: 403);
        }
      }
    }
    ...
    if(response?.statusCode == 200){
      ...
      // No dialog is open here (the confirm dialog pops before the API call),
      // so a Get.back() would pop the dashboard route on failure.
      isLoading = false;
