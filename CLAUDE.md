...
1. **PIN path (primary Vito flow):** username + 6-digit PIN. New users must first validate a QR/invite token (`POST /api/auth/qr-token/validate`). Routes in `Routes/api.php` under `VitoAuthController`. Tokens: 1-hour expiry for customers, 7-day for drivers. PIN change revokes all other active sessions. Invite deep links (v3.8.5+): `landing/index.html` opens `https://dacatlon.store/locate-user|/locate-driver?token=…` (role-aware, Android `intent://` with Play fallback); both apps' `login_helper.dart` extract the `token` query param from the initial App Link and open `TokenGateScreen(initialToken:)` pre-validated — a link *without* `?token=` stays the live-location share path.
2. **OTP path (alternative):** phone number + SMS OTP via `ClientOtpAuthController` (`send-otp` → `otp-verification` → `registration-from-otp`). OTPs are bcrypt-hashed in `vito_otps`, 5-min expiry, 30s resend cooldown, 5-attempt lock. Not gate-guarded (no QR required). **Test-only predictable OTP:** the single number in `config('services.vito_test_otp.phone')` (`VITO_TEST_OTP_PHONE`, default `+18885550000`) always gets the fixed `VITO_TEST_OTP_CODE` (default `123456`) with no SMS; every other number keeps a secure random OTP. `verifyOtp` is unchanged (still hash-checked/expiring/attempt-limited). `TestCustomerSeeder` puts that number on `testcustomer`. Disable with `VITO_TEST_OTP_PHONE=` (empty). **Fail-closed in production:** under `APP_ENV=production` the feature is OFF unless `VITO_TEST_OTP_ALLOW_PRODUCTION=true` is set explicitly.
...
- **Pending-ride auto-cancel:** handled by the scheduled `trip-request:cancel` command (`CancelPendingTrips`, wired `everyMinute()` in `app/Console/Kernel.php`); requires the Laravel scheduler cron (`php artisan schedule:run`) to be running. Push/broadcast jobs (`app/Jobs/*PushNotification*`) need `QUEUE_CONNECTION=redis` + a running `php artisan queue:work` (or Supervisor); file/sync-queue mode drops them silently.
